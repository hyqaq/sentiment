    ...
    with torch.no_grad():
        ...
        for batch in test_loader:
           ...
           # print(predicted,labels)
            ...
            print("In  this %d test batch: Accuracy:   %.3f %%,Loss:   %.6f"%(ii+1,(predicted==labels).sum().item(),output[0].mean()))
            ii+=1
            # if (pred==target):
            #     print(pred)
...
for epoch in range(3):
    ...
    for batch_idx,batch in enumerate(train_loader,0):
        ...
        if batch_idx%10==9:
            time_end=time.time()
            print("Epoch:{}      [{}/{}({:.0f}%)]\tTraining Loss: {:.6f}  consuming time：{:.3f}".format(
                epoch,batch_idx+1,len(train_loader.dataset)/batch_size,100.*batch_idx/len(train_loader),
                running_loss/10,time_end-time_begin
            ))
            running_loss=0.0
            time_begin = time.time()
    # path='model/'+str(epoch)+'my_model.pth'
    # torch.save(model.state_dict(),path)
    path = 'model/' + str(epoch) + 'my_model9161626.pth'
